...
BOOT_CODE static int map_kernel_region(pte_t *lvlnpt, int pt_level, word_t region_size, pptr_t pptr, paddr_t paddr, int free_pt_index)
{
    ...
    if (IS_ALIGNED(paddr, RISCV_GET_LVL_PGSIZE_BITS(pt_level))) {
        ...
        assert(IS_ALIGNED(pptr, RISCV_GET_LVL_PGSIZE_BITS(pt_level)));


        /* Map leaf pages */
        for (int pt_index = 0; pt_index < num_required_entries; pt_index++) {
            lvlnpt[RISCV_GET_PT_INDEX(pptr, pt_level)] =
                pte_new(
                    paddr >> seL4_PageBits,
                    0,  /* sw */
                    1,  /* dirty */
        ...
        /* The paddr isn't aligned to pt_level, try to allocate a ++pt_level page table and map entries into it */
    } else {
